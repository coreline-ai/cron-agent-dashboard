# Multica 디자인 코드 레벨 분석

작성 일시: `2026-05-13 19:39 KST`

대상 참조: `/Users/hwanchoi/projects/multica/multica-main`  
적용 대상: `/Users/hwanchoi/projects/core-agent-dashboard`

> 원본 Multica 폴더는 읽기 전용으로 분석만 수행했다. 실제 산출물은 본 저장소의 `docs/design/`와 `web/src/`에 작성한다.
...
Multica는 장식적인 SaaS 랜딩 스타일이 아니라 **업무 밀도 높은 도구 UI**를 지향한다.
...
Multica의 핵심 token은 shadcn naming을 따른다.
...
Corn 적용: `DashboardLayout`의 sidebar를 `PRODUCT / WORKSPACE` 그룹으로 분리하고, current active route가 Multica식 active state로 보이게 한다.

### 4.2 Page header

Multica `PageHeader`는 큰 hero가 아니라 48px compact bar이다.
...
- Multica의 팀 기능/초대/프로젝트/인박스 UX는 가져오지 않는다.
