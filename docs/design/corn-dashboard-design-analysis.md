# Corn Design Reference 디자인 코드 레벨 분석
...
> 원본 Corn Design Reference 폴더는 읽기 전용으로 분석만 수행했다. 실제 산출물은 본 저장소의 `docs/design/`와 `web/src/`에 작성한다.
...
Corn Design Reference는 장식적인 SaaS 랜딩 스타일이 아니라 **업무 밀도 높은 도구 UI**를 지향한다.
...
Corn Design Reference의 핵심 token은 shadcn naming을 따른다.

| Token | Light | Dark | Corn 적용 |
|---|---:|---:|---|
...
| 역할 | Corn Design Reference 규칙 | Corn 적용 |
|---|---|---|
...
Corn Design Reference sidebar는 다음 구조가 핵심이다.
...
Corn 적용: `DashboardLayout`의 sidebar를 `PRODUCT / WORKSPACE` 그룹으로 분리하고, current active route가 Corn Design Reference식 active state로 보이게 한다.

### 4.2 Page header

Corn Design Reference `PageHeader`는 큰 hero가 아니라 48px compact bar이다.
...
Corn 적용: 기존 대형 `h1 clamp(4.5rem)`를 제거하고 `page-header`를 compact 정보 헤더로 전환한다.

### 4.3 Cards / panels

Corn Design Reference issue card:
...
Corn 적용: `.button`, `.button.secondary`, `.button.danger`, inputs를 이 규칙에 맞춤.

## 5. 기존 Corn UI와 차이

| 항목 | 기존 Corn | Corn Design Reference 기준 | 조치 |
|---|---|---|---|
...
- Corn Design Reference의 팀 기능/초대/프로젝트/인박스 UX는 가져오지 않는다.
