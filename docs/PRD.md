...
기존 Corn Design Reference는 팀/조직을 가정한 풀스택 시스템이라 단일 사용자에게는 과한 토큰 소비와 복잡도가 발생한다.
corn-agent-dashboard는 Corn Design Reference의 검증된 UX(보드/댓글/위임)를 보존하면서 단일 사용자에게 불필요한 모든 기능을 제거한다.

---

## 2. Problem Statement (해결할 문제)

### 2.1 현재 Corn Design Reference의 문제 (single user 관점)
...
- M7. **현재 Corn Design Reference 테마/스타일 유지** — 다크모드, 한국어, shadcn 컴포넌트
...
- "Corn Design Reference 켤까 말까" 고민 없이 바로 켜진다
...
2. **에이전트 결과 markdown 렌더링**: 기존 Corn Design Reference에서 추출 vs `react-markdown` 신규? → 추출
