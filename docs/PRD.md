...
기존 Cron Design Reference는 팀/조직을 가정한 풀스택 시스템이라 단일 사용자에게는 과한 토큰 소비와 복잡도가 발생한다.
cron-agent-dashboard는 Cron Design Reference의 검증된 UX(보드/댓글/위임)를 보존하면서 단일 사용자에게 불필요한 모든 기능을 제거한다.

---

## 2. Problem Statement (해결할 문제)

### 2.1 현재 Cron Design Reference의 문제 (single user 관점)
...
- M7. **현재 Cron Design Reference 테마/스타일 유지** — 다크모드, 한국어, shadcn 컴포넌트
...
### 4.3 Won't (W) — v0.1 MVP 기준 명시적 제외

> 2026-05-23 note: 일부 Phase 2 후보(Attachment, 부모/서브 이슈 트리)는 현재 구현됨. 이 절은 초기 MVP boundary 기록으로 보존한다.
...
- W8. ❌ Priority/Label (Attachment는 Phase 2에서 구현됨)
- W9. ❌ pgvector/embedding/semantic search
- W10. ❌ **부모/서브 이슈 트리** (MVP 제외, Phase 2에서 구현됨) — MVP에서는 위임 체인을 같은 이슈의 댓글/run 누적으로 표현했다.
...
- "Cron Design Reference 켤까 말까" 고민 없이 바로 켜진다
...
1. **인증**: 완전 무인증 vs 단일 토큰(`CRON_AGENT_DASHBOARD_TOKEN`)? → 기본 무인증, 토큰은 옵션
2. **에이전트 결과 markdown 렌더링**: 기존 Cron Design Reference에서 추출 vs `react-markdown` 신규? → 추출
