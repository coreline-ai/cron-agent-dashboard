...
| cancelled | `POST /api/issues/:id/cancel`, 재시작 시 orphan 정리, graceful shutdown 중 강제 종료, 이슈 자체 cancel 시 queued run 자동 취소 |
...
- in-memory mutex 불필요. 부팅 시 살아남는 상태가 없음 → 더 단순.
...
- exec 실패 → issue.status=failed (5xx 아님, 정상 동작)

### 6.3 부분 실패
- 멘션 파싱 실패 → 댓글은 저장, dispatch만 skip + 경고 댓글 추가
- Autopilot 룰 실행 실패 → 룰은 살아있고 issue가 failed로 기록
...
## 8. Multica 대비 차이 (검증된 결정만 가져옴)

| Multica 결정 | corn-agent-dashboard 채택? | 이유 |
|---|---|---|
...
| Next.js (Multica) | ❌ → Vite | SSR/RSC 미사용 + static export 동적 라우트 회피 |
