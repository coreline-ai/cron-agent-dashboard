...
3. executor 시작/출력 절단/취소 요청/완료/실패/복구를 `run_event`에 append.
4. UI는 `GET /api/runs/:id/events`로 Issue Detail의 이벤트 타임라인을 표시한다.
5. stale scanner는 일정 시간 heartbeat가 멈춘 running run을 `cancelled + stale_recovered`로 복구한다.
