# ARCHITECTURE — cron-agent-dashboard
...
└─────────────────────────────┬───────────────────────────────────┘
                              ...
                              ▼
┌─────────────────────────────────────────────────────────────────┐
│              cron-agent-dashboard binary (단일 프로세스)                   │
│                                                                 │
│  ┌──────────────────────────────────────────────────────┐       │
...
- 사용자 댓글 멘션은 항상 explicit dispatch다. agent 결과 댓글 안의 `@AgentName`은 기본적으로 실행하지 않으며, workspace `auto_chain_enabled=true`일 때만 별도 guard(depth/run/cost/dry-run)를 통과해 자동 dispatch된다.
- Auto-chain run은 `trigger_type='mention'`을 유지하고 `parent_run_id`/`chain_id`/`chain_depth`와 `run_event.details.auto_chain=true`로 구분한다.
...
robfig/cron Scheduler (in-process, 등록 방식)
  ...
  - cron.Location = CRON_AGENT_DASHBOARD_TIMEZONE (기본 Asia/Seoul)
...
### 5.1 직렬화 (per-issue + workspace 기본 직렬 / per-run worktree opt-in)
- **DB 레벨로 직렬화**: claim 쿼리에서 보장
  - **per-issue**: 같은 이슈에 동시 running 없음 (항상 적용)
  - **per-workspace 기본값**: `workspace.per_run_worktree=false`이면 같은 워크스페이스에 동시 running 없음
  - **per-run worktree opt-in**: `workspace.per_run_worktree=true`이면 같은 workspace의 다른 issue도 병렬 claim 가능. worker는 run별 cwd를 `<data_dir>/worktrees/<slug>/<run-id>/`로 바꾼다.
- 워크스페이스 기본 직렬화 근거: 같은 `workspace.working_dir`에서 두 에이전트가 동시 실행 시 파일 충돌, git 상태 꼬임 위험. 안전 기본값은 유지한다.
- git repo workspace는 `git worktree add --detach` / `git worktree remove --force`를 사용하고, 비-git 경로는 0700 isolated directory로 fallback한다.
...
- claim 쿼리:
    ...
    SELECT r.id, r.issue_id FROM run r
     ...
     WHERE r.status='queued'
       ...
       AND (COALESCE(w.per_run_worktree, 0) = 1
            OR NOT EXISTS (SELECT 1 FROM run r3 JOIN issue i3 ON i3.id=r3.issue_id
  ...
  - `per_run_worktree=false`인 workspace에서 다른 issue들이 같은 시점에 NOT EXISTS를 통과해도, UPDATE 단계의 BEGIN IMMEDIATE write lock + WHERE guard로 한쪽만 성공한다. 다른 워커는 다음 polling에 다시 SELECT한다.
- pool size 변경: 재시작 필요 (Phase 1). default 3.
- 1초 polling은 SQLite WAL에 거의 부담 없음 (read-only SELECT, lock 없음)
- 기본값은 worker pool size 3이라도 같은 워크스페이스에서는 사실상 직렬. `per_run_worktree=true`인 workspace는 같은 workspace의 다른 issue도 병렬 실행될 수 있으므로 `--workers`를 함께 조정한다.
...
| 첨부 파일 | 구현됨. `attachment`/`attachment_audit` 테이블 + `{data_dir}/attachments` 저장 + multipart HTTP/UI |
| 외부 봇 통합 | 이미 REST API가 외부 호출 가능 → 토큰만 발급 |
| 이슈 webhook | 구현됨. `webhook`/`webhook_delivery` 테이블 + HMAC-SHA256 + retry/dead-letter |
| Auto-chain opt-in | 구현됨. 기본 off, workspace opt-in + `chain_id`/`parent_run_id`/`chain_depth` + depth/run/cost/dry-run guard + chain dashboard |
| Realtime streaming | 구현됨. 이슈 상세는 `/api/issues/{id}/events/stream` SSE + fetch 기반 subscriber로 run_event를 수신하고, run 단위 소비자는 `/api/runs/{id}/events/stream`을 사용할 수 있다. React Query polling은 fallback/보드 refresh 용도로 유지 |
| 다국어 | 한국어 string을 i18n 파일로 추출 (지금은 inline) |
| 멀티 사용자 | 본 프로젝트 범위 외 (별도 fork) |

---

## 8. Cron Design Reference 대비 차이 (검증된 결정만 가져옴)

| Cron Design Reference 결정 | cron-agent-dashboard 채택? | 이유 |
|---|---|---|
| Polymorphic assignee (`type,id`) | ❌ | 멤버 없음 → agent_id 단일 컬럼 |
| WebSocket Hub | ❌ | SSE + polling fallback으로 충분 (단일 사용자) |
...
| Next.js (Cron Design Reference) | ❌ → Vite | SSR/RSC 미사용 + static export 동적 라우트 회피 |
...
## 9. 체이닝 정책과 auto-chain

현재 제품 정책은 **사용자 명시 멘션 우선(explicit-first) + workspace opt-in auto-chain**이다.

- 사용자 댓글의 `@AgentName`은 같은 issue에 새 run을 만든다.
- agent 결과 댓글의 `@AgentName`은 기본적으로 markdown 텍스트로만 보존한다.
- workspace `auto_chain_enabled=true`일 때만 완료된 agent 결과 댓글의 첫 mention이 자동 dispatch 후보가 된다.
- 이 정책은 무한 루프, 예상치 못한 비용 폭주, agent hallucinated mention 실행을 기본 차단하기 위한 안전장치다.
...
|---|---|
| 기본 상태 | OFF |
| 최대 chain depth | `workspace.auto_chain_max_depth` |
| 24시간 run 제한 | `workspace.auto_chain_daily_run_limit`, `0`이면 제한 없음 |
| 24시간 비용 제한 | `workspace.auto_chain_daily_cost_micros`, `0`이면 제한 없음 |
| dry-run | 감지와 system comment만 남기고 enqueue 생략 |
| 같은 chain 내 동일 agent 재호출 | 차단 |
| source run 상태 | `done`일 때만 chain 생성 |
| lineage 필드 | `chain_id`, `parent_run_id`, `chain_depth` |
| trigger_type | 기존 enum 호환을 위해 `mention` 유지, `run_event.details.auto_chain=true`로 구분 |

상세 정책은 [CHAINING.md](CHAINING.md)를 참조한다.
...
- Runtime adapters parse best-effort metrics from stderr/structured metrics output. Agent stdout is treated as user-controlled content and excluded from metrics parsing. Captured values are stored on `run.input_tokens`, `run.output_tokens`, `run.total_cost_micros`, and `run.model_resolved`.
