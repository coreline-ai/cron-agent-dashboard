...
- [ ] **Frontend 스택 = Vite + React Router SPA (확정)**
...
- [ ] **시스템 timezone 정책**: 환경변수 `CORN_AGENT_DASHBOARD_TIMEZONE` (기본 `Asia/Seoul`)
- [ ] **꺼져 있는 동안의 cron**: 누락된 시각은 실행 안 함 (robfig 기본 동작)
- [ ] **stdout cap**: 단일 run 최대 10MB (초과 시 truncation + 경고)
- [ ] **Worker 동시성 단위**: 전역 풀 (워크스페이스별 격리는 Phase 2)

### Tasks
- [ ] `go mod init github.com/hwanchoi/corn-agent-dashboard`
- [ ] 디렉토리 구조 생성 (`cmd/`, `internal/`, `web/`, `docs/`)
- [ ] `web/` Vite + React + TS + Tailwind v4 init
- [ ] `.gitignore`, `.editorconfig`, `Makefile` 추가
- [ ] `cmd/corn-agent-dashboard/main.go` — hello world
- [ ] GitHub Actions: lint (golangci-lint, eslint) + build

### 완료 기준
- `go build ./...` 성공
- `pnpm --filter web build` 성공 (Vite 빌드)
- CI 그린
- 위 의사결정 항목 모두 TRD에 반영됨
...
- [ ] `internal/db/db.go` — sqlite open, pragma, migrate
- [ ] `internal/db/migrations/0001_init.sql` — 6 테이블 + meta
  - **run 테이블에 `status / claimed_at / claimed_by` 포함** (durable queue)
  - issue.status 도메인: `queued | running | done | failed | cancelled`
  - workspace.working_dir 컬럼 포함
  - agent 테이블 case-insensitive unique 인덱스
- [ ] `internal/db/migrations/0002_indexes.sql`
- [ ] 부팅 시 migration 자동 적용 + idempotent
- [ ] 부팅 시 orphan 정리: `run.status='running' AND finished_at IS NULL` → `cancelled` + error_message
- [ ] 단위 테스트: 두 번 부팅해도 멱등

#### P1.2 Store Layer (2일)
- [ ] `internal/store/workspace.go` — Create/Get/List/Update/Delete
- [ ] `internal/store/agent.go` — 동일 + Promote
- [ ] `internal/store/issue.go` — 동일 + identifier 발급 (트랜잭션)
- [ ] `internal/store/comment.go`
- [ ] `internal/store/run.go`
- [ ] `internal/store/autopilot.go`
- [ ] 각 store별 단위 테스트 (in-memory sqlite)

#### P1.3 HTTP Layer (1.5일)
- [ ] `internal/http/router.go` — chi 라우팅 등록
- [ ] 핸들러 30개 (API.md 명세 그대로)
- [ ] 에러 응답 형식 통일 (`internal/http/errors.go`)
- [ ] 입력 검증 (validator 또는 inline)
- [ ] CORS / 토큰 미들웨어

#### P1.4 통합 테스트 (0.5일)
- [ ] httptest로 핸들러 통합 테스트 (워크스페이스 생성 → 이슈 생성 → 댓글 조회)
- [ ] 에러 경로 (validation, 404, conflict)
...
- [ ] `internal/worker/runtime/adapter.go` — `RuntimeAdapter` 인터페이스 (TRD §1.4 참조)
  - [ ] `Name() string`
  - [ ] `Detect(ctx) RuntimeInfo` — PATH/version 검출
  - [ ] `BuildCommand(ctx, RunContext) (*exec.Cmd, []byte stdin, error)` — CLI별 인자/stdin/non-interactive 모드 분기
- [ ] `internal/worker/runtime/codex.go`, `claude.go`, `gemini.go` 구현
- [ ] `internal/worker/executor.go`
  - [ ] `cmd.SysProcAttr.Setpgid = true` (process group)
...
- [ ] 단위 테스트:
  ...
  - [ ] cancel 시 process group kill (자식의 자식까지 정리)
...
- [ ] graceful shutdown:
  - [ ] 신규 claim 정지
  - [ ] 진행 중 run에 SIGTERM (자식 프로세스 group kill)
  - [ ] 최대 30초 대기, 미종료 시 SIGKILL
  - [ ] run.status='cancelled' + error_message="shutdown" 기록
...
#### P2.4 멘션 파싱 (0.5일)
- [ ] `internal/worker/mention.go` — 정규식 `@([A-Za-z0-9_\-가-힣]+)`
...
- [ ] `run.status='running' AND finished_at IS NULL` → `cancelled` + error_message="orphan recovered"
...
- [ ] `internal/scheduler/cron.go`
  - [ ] robfig/cron 통합
  - [ ] 부팅 시 DB에서 enabled 룰 전부 로드
  - [ ] 룰 CRUD 시 scheduler reload (전체 reload, 단순)
- [ ] 시각 도래 시:
  - [ ] 템플릿 변수 치환 (`{{date}}`, `{{datetime}}`, `{{time}}`)
  - [ ] issue INSERT (created_by=autopilot, autopilot_rule_id 채움)
  - [ ] worker enqueue
  - [ ] rule.last_run_at / next_run_at 업데이트
- [ ] 수동 트리거 엔드포인트 (`POST /autopilot/:id/trigger`)
- [ ] cron expression 검증 (룰 생성/수정 시)
...
- [ ] API client (`web/lib/api.ts`) — fetch wrapper + 에러 처리
- [ ] TanStack Query 셋업
...
#### P4.2 워크스페이스 페이지 (1일)
- [ ] `/` — 목록 + 빈 상태
- [ ] 새 워크스페이스 모달 (메인 에이전트 동시 생성)

#### P4.3 이슈 보드 (1일)
- [ ] `/w/:slug/board` — 카드 list
...
- [ ] `/w/:slug/issues/:id`
- [ ] 본문 + 댓글 스레드 (markdown 렌더)
- [ ] 사이드바 (상태/담당/작업/run 이력)
- [ ] 댓글 입력 (@AgentName 자동완성 — Phase 4 nice-to-have)
- [ ] polling 3초 (running일 때만)
- [ ] 재실행 / 취소 / 편집 / 삭제

#### P4.5 에이전트 페이지 (0.5일)
- [ ] `/w/:slug/agents` — list + 추가 모달
- [ ] `/w/:slug/agents/:id` — 편집 폼

#### P4.6 Autopilot 페이지 (1일)
- [ ] `/w/:slug/autopilot` — 룰 카드 + on/off
- [ ] 룰 생성/편집 모달 (preset + advanced cron)
- [ ] [지금 실행] / [삭제]

#### P4.7 설정 페이지 (0.5일)
- [ ] `/settings` — 버전 + 런타임 가용성 + 워커 수 + 인증 모드 + 외관
...
### Tasks
- [ ] `vite build` → `web/dist/` 정적 산출물
- [ ] `go:embed web/dist/*` 로 Go에 임베드
- [ ] HTTP 라우터에 정적 핸들러 등록:
  - [ ] `/api/*` → API 핸들러
  - [ ] 그 외 모든 경로 → `index.html` fallback (SPA client routing)
  - [ ] 정적 자산(`*.js`, `*.css`, `assets/*`)은 그대로 서빙
- [ ] 빌드 스크립트 (`make build` → vite build → go build)
- [ ] 단일 바이너리에서 `/`, `/w/:slug/board`, `/api/*` 모두 동작 확인
- [ ] **static routing smoke test**: `/w/foo/issues/NEWS-1` 직접 접근(=새로고침) 시 index.html fallback → client routing 작동 검증
...
### Tasks
- [ ] **부팅 자가검진**
  - [ ] 워크스페이스마다 메인 에이전트 정확히 1개 확인
  - [ ] orphan run 정리
  - [ ] DB pragma 확인
- [ ] **백업/복구**
  - [ ] `corn-agent-dashboard backup --to <path>` 명령
  - [ ] `corn-agent-dashboard restore --from <path>` 명령
- [ ] **로그 관리**
  - [ ] `--log-retention-days N` 옵션
  - [ ] 부팅 시 N일 이상 된 stdout 파일 삭제
- [ ] **에러 핸들링 강화**
  - [ ] DB lock 충돌 시 재시도
  - [ ] worker panic recover
...
### Tasks
- [ ] 크로스 컴파일 (darwin/arm64, darwin/amd64, linux/amd64, linux/arm64)
- [ ] GitHub Release CI
- [ ] README 마무리 (스크린샷, 빠른 시작)
