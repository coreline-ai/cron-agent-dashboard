...
- [x] `internal/worker/executor.go`
  ...
  - [x] stdout 캡처 → 파일 append + 10MB cap (cap 후에도 `io.Discard`로 drain — pipe blocking 방지)
  - [x] stderr → 메모리 ring buffer (마지막 4KB) → error_message
  - [x] 타임아웃 600초 (context 기반) — `RuntimeExecutor.Timeout` default 10m
  - [x] cancel 지원: `syscall.Kill(-pgid, SIGTERM)` → 30초 후 SIGKILL — `executor_process_unix.go`
- [x] Phase 1 결정: stdout 결과 댓글은 종료 후 1번 INSERT (64KB cap, 초과 시 로그 링크 append) — `CapCommentForLogWithStatus`
- [x] 단위 테스트:
  - [x] echo 명령으로 정상 실행 — `executor_test.go`
  - [x] 10MB 초과 stdout drain 확인 (sleep + 무한 출력 시나리오) — `executor_test.go`
  - [x] cancel 시 process group kill (자식의 자식까지 정리)

#### P2.2 Worker Pool (durable queue, 1.5일)
- [x] `internal/worker/pool.go` skeleton
  - [x] N goroutine이 각각 `BEGIN IMMEDIATE; SELECT FROM run WHERE status='queued' ORDER BY started_at LIMIT 1; UPDATE run SET status='running', claimed_at, claimed_by; COMMIT;` 패턴으로 claim — `internal/store/runs.go::ClaimNextRun`
  - [x] queue 비면 1초 sleep 후 재폴링 (1초 정밀도면 충분) — pool poll loop
  - [x] per-issue 직렬화: claim 시 같은 issue_id에 이미 `running` run 있으면 skip (자연스러운 직렬화) — claim 가드
  - [x] panic recover (run 정리 + 에러 메시지 기록) — `pool.go` defer recover
- [x] HTTP 핸들러는 channel enqueue가 아니라 **run row INSERT(status='queued')** 만 하면 끝 — decision
- [x] graceful shutdown:
  - [x] 신규 claim 정지 — pool ctx.Done
...
- [x] 템플릿: agent.instructions + issue.title + issue.body + 최근 댓글 3개 — `worker.RenderPrompt`
- [x] 댓글 컨텍스트 길이 제한 (4000 chars) — `capSnapshot` UTF-8-aware

#### P2.4 멘션 파싱 (0.5일)
- [x] `internal/worker/mention.go` — 정규식 `@([\p{L}\p{N}_\-]+)`
- [x] 댓글 POST 시 호출 → 에이전트 매칭 (대소문자 무시, `lower(name)`) — `dispatchMentionRun`
- [x] **첫 멘션만 사용** (multiple → 첫 번째 + system warning) — decision
- [x] 매칭 성공: 새 run INSERT (`trigger_type='mention'`, `trigger_comment_id`, `trigger_content_snapshot`). **issue.assignee_agent_id는 변경하지 않음**.
- [x] 매칭 실패: warning system 댓글 추가
- [x] 중복 dispatch (같은 issue, agent에 queued 존재): `idx_run_one_queued_per_issue_agent` unique 위반 → warning
...
- [x] 해당 이슈 → 같은 trigger로 새 run을 자동 생성 (재개) 또는 사용자가 수동 재실행 — superseded: 자동 재개는 무한 루프 위험으로 MVP에서 제외
- [x] **결정**: MVP는 **자동 재개 안 함**. 사용자가 보드에서 confirm 후 [재실행] 클릭. (자동 재개는 무한 루프 위험)
...
#### P4.0 셋업 (0.5일)
- [x] Tailwind config (Cron Design Reference에서 복사) — superseded: 단일 `global.css` skeleton 유지 결정 (상단 callout 참조)
- [x] shadcn 컴포넌트 초기 셋업 (button/dialog/input/textarea/select/badge) — superseded: shadcn 없이 직접 컴포넌트화
- [x] 다크모드 (class 기반 토글, next-themes 미사용) — superseded: v0.1.x는 다크-only skeleton 유지
- [x] API client (`web/src/api/client.ts`) — fetch wrapper + 에러 처리
- [x] TanStack Query 셋업

#### P4.1 레이아웃 (0.5일)
- [x] 헤더 (로고 + 워크스페이스 드롭다운 + 설정 + 다크모드) — 다크모드 토글은 superseded, 나머지 구현됨
- [x] 사이드바 (보드/에이전트/자동화) — `WorkspaceLayout` 구현
- [x] 라우팅 구조
...
- [x] status 필터 / agent 필터 / 검색 — `BoardPage` 구현
- [x] [+ 새 이슈] form
- [x] polling 5초
...
- [x] 사이드바 (상태/담당/작업/run 이력) — `IssueDetailPage` 사이드바
...
- [x] **로그 관리**
  - [x] `--log-retention-days N` 옵션 — `--auto-cleanup-log-days`로 명명 변경
  - [x] 부팅 시 N일 이상 된 stdout 파일 삭제 — `MaintenanceRunner`가 주기 실행 + 부팅 직후 1회
  - [x] runtime CLI stdout noise 모니터링 — `internal/worker/runtime/sanitize.go::knownNoiseLines`. 현재 1개 패턴(`MCP issues detected. Run /mcp list for status.`) 등록. 새 패턴 발견 시 추가만 하면 됨.
- [x] **에러 핸들링 강화**
...
- [x] 첫 사용자 시나리오 영상 / 스크린샷 — 스크린샷 `make screenshots`로 생성 가능, 영상은 외부 deferred
