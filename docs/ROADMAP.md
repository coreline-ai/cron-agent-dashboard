...
- P0 골격, Go DB/API skeleton, worker/runtime/scheduler skeleton, Vite 7-route skeleton 구현 완료.
- `go test ./...`, `go build ./...`, `pnpm --filter web build`, `make check`로 검증한다.
- 남은 통합 작업: worker pool과 store/main 연결, scheduler DB reload 연결, Go `embed.FS` static serving, release build.
...
- [x] `internal/worker/runtime/codex.go`, `claude.go`, `gemini.go` skeleton 구현
...
#### P2.4 멘션 파싱 (0.5일)
- [x] `internal/worker/mention.go` — 정규식 `@([A-Za-z0-9_\-가-힣]+)`
