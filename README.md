...
[![Status](https://img.shields.io/badge/status-foundation%20scaffold-blue?style=for-the-badge)](docs/ROADMAP.md)
...
> [!IMPORTANT]
> **이 프로젝트는 현재 초기 구현 스캐폴드 단계입니다.** Go SQLite/API 기반, worker/scheduler 골격, Vite React 7-route UI가 추가되었고 `make check`로 자체 검증합니다. 단일 바이너리 static embed, 실제 worker main-loop 연결, 릴리스 패키징은 후속 Phase입니다.
...
> [!NOTE]
> 릴리스 패키지는 아직 없습니다. 현재는 **개발 실행** 기준으로 백엔드 API와 Vite UI skeleton을 확인할 수 있습니다.
...
# 또는 직접 다운로드
curl -L https://github.com/coreline-ai/cron-agent-dashboard/releases/latest/download/corn-agent-dashboard-$(uname -s)-$(uname -m) \
...
| **P2** | 에이전트 실행 | Runtime adapter · worker pool · 멘션 파싱 | 2~3 | 🧱 골격 구현 |
| **P3** | Autopilot | cron + 룰 CRUD + 수동 트리거 | 1~2 | 🧱 골격 구현 |
| **P4** | Frontend | 7 페이지 (Vite + React Router SPA) | 5~7 | 🧱 라우트 skeleton |
...
초기 구현 단계라 외부 기여는 아직 받지 않음. Phase 7 릴리스 후 재검토 예정.
