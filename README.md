...
- [ ] 🌲 부모 / 서브 이슈 트리
- [ ] 🔗 Auto-chain opt-in — 기본 off 후보 (`chain_id`/`parent_run_id`/`chain_depth` 설계 필요)
- [ ] 📎 첨부 파일
...
> [!IMPORTANT]
> 현재 체이닝 정책은 **explicit-only**입니다. 사용자가 직접 작성한 댓글의 `@AgentName` 명시 멘션만 새 run을 만들고, agent 결과 댓글 안의 `@AgentName`은 자동 dispatch하지 않습니다. 이는 무한 루프, 예상치 못한 비용 폭주, hallucinated mention 실행을 막기 위한 MVP 안전 정책입니다. Auto-chain은 기본값 off인 Phase 2+ opt-in 후보이며 현재 기능이 아닙니다.
...
6. **체이닝은 explicit-only** — 사용자 댓글의 명시 멘션만 dispatch. agent 결과 멘션 자동 실행은 Phase 2+ opt-in 후보(기본 off)
