...
[![Status](https://img.shields.io/badge/status-design%20stage-yellow?style=for-the-badge)](docs/ROADMAP.md)
...
[![Tailwind CSS](https://img.shields.io/badge/Tailwind%20CSS-v4-38B2AC?style=flat-square&logo=tailwindcss&logoColor=white)](https://tailwindcss.com/)
...
> [!IMPORTANT]
> **이 프로젝트는 현재 설계 단계입니다** (Phase 0 시작 전). 모든 문서는 [`docs/`](docs/)에 있고, 구현은 아직 들어가지 않았습니다.
...
- [ ] 🗂️ 워크스페이스 CRUD (이슈 ID prefix · 기본 working_dir)
- [ ] 🤖 메인 에이전트 1개 + 추가 에이전트 N개 (case-insensitive 이름 유일)
- [ ] 🎫 이슈 트래커 (`identifier` `NEWS-12`, status: `open` / `done` / `cancelled`)
- [ ] 💬 댓글 스레드 + system 댓글 (실행 시작 / 취소 / 경고)
- [ ] 🔀 `@AgentName` 멘션 위임 — **assignee는 바꾸지 않음**, 같은 이슈에 새 run 추가
- [ ] ⏰ Autopilot (robfig/cron, 시스템 timezone `Asia/Seoul` 기본)
- [ ] 🔁 [재실행] — 마지막 run의 agent로 자동 dispatch
- [ ] 🛑 [취소] — process group SIGTERM → 30초 후 SIGKILL
...
<tr>
<td>Backend 언어</td>
<td><img src="https://img.shields.io/badge/Go-1.23%2B-00ADD8?style=flat&logo=go&logoColor=white"/></td>
...
<tr>
<td>UI 라이브러리</td>
<td><img src="https://img.shields.io/badge/shadcn%2Fui-000?style=flat"/> <img src="https://img.shields.io/badge/Tailwind-v4-38B2AC?style=flat&logo=tailwindcss&logoColor=white"/></td>
<td>Multica 디자인 토큰 재사용</td>
</tr>
<tr>
<td>상태 관리</td>
...
<tr>
<td>라우팅</td>
<td><img src="https://img.shields.io/badge/React%20Router-6-CA4245?style=flat&logo=reactrouter&logoColor=white"/></td>
<td>SPA 동적 라우트</td>
</tr>
<tr>
<td>Markdown</td>
<td><img src="https://img.shields.io/badge/react--markdown-+%20remark--gfm-9C27B0?style=flat"/></td>
<td>raw HTML 금지 (인젝션 방어)</td>
</tr>
<tr>
<td>Storage</td>
...
> [!NOTE]
> 아직 구현되지 않았습니다. 아래는 **목표하는 사용자 경험**입니다. Phase 7 릴리스 후 실제 동작합니다.
...
```bash
# Homebrew (예정)
brew install hwanchoi/tap/corn-agent-dashboard

# 또는 직접 다운로드
curl -L https://github.com/hwanchoi/corn-agent-dashboard/releases/latest/download/corn-agent-dashboard-$(uname -s)-$(uname -m) \
...
| `--log-retention-days` | — | (off) | run stdout 파일 자동 삭제 일수 |
...
cp ~/.corn-agent-dashboard/data.db ~/backup/data.db.$(date +%Y%m%d)
cp -r ~/.corn-agent-dashboard/runs ~/backup/runs

# 복구
cp ~/backup/data.db ~/.corn-agent-dashboard/data.db
...
|---|---|---|---|---|
| **P0** | 프로젝트 셋업 + 핵심 의사결정 | go mod · Vite skeleton · CI · RSC 스캔 | 1~2 | 📐 설계 완료 |
| **P1** | 백엔드 코어 | DB + store + 33 REST API | 3~5 | ⏳ 대기 |
| **P2** | 에이전트 실행 | Runtime adapter · worker pool · 멘션 파싱 | 2~3 | ⏳ 대기 |
| **P3** | Autopilot | cron + 룰 CRUD + 수동 트리거 | 1~2 | ⏳ 대기 |
| **P4** | Frontend | 7 페이지 (Vite + React Router SPA) | 5~7 | ⏳ 대기 |
| **P5** | 통합 / 임베드 | static export + embed.FS + 단일 바이너리 | 1 | ⏳ 대기 |
| **P6** | 품질 / 운영 | 부팅 자가검진 · 백업 / 복구 · 성능 검증 | 2 | ⏳ 대기 |
| **P7** | 릴리스 | 크로스 컴파일 · README 스크린샷 · 데모 | 1 | ⏳ 대기 |
...
5. **멘션 ≠ assignee 변경** — 멘션은 일회성 위임이며 `run.agent_id`만 다르게
...
설계 단계라 외부 기여 받지 않음. Phase 7 릴리스 후 재검토 예정.
