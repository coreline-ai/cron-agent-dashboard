...
- **2026-05-24 multi-agent lab cycle**: added `cron-agent-dashboard seed-lab` to provision a 7-workspace / 22-agent 실개발 테스트 lab with per-run worktrees, auto-chain guards, and one `[LAB]` project issue per workspace. Added `cron-agent-dashboard seed-dev-team` for a single-workspace 7-role AI dev team (Lead/Designer/Backend/Frontend/DB/QA/DevOps) with 8 Agent Skills. Added `docs/MULTI_AGENT_LAB.md` and `docs/dev-team-workflow.md` runbooks.
- **2026-05-24 lab integration cycle**: integrated selected multi-agent lab output for the CoreMCP-style demo home layout, refreshed README screenshots, added run-scoped run_event SSE (`/api/runs/:id/events/stream`) with token-mode coverage, and expanded worktree disk/GC Settings visibility + operations runbook.
