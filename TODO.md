...
- [ ] codex `--json` 전환 — JSON Lines 파서 + `ParseMetricsFromText` 재작성. 호환성 영향 큼.
...
- [ ] `knownNoiseLines` 모니터링 — 새 runtime CLI 진단 패턴 발견 시 추가 (현재 1개 `MCP issues detected. Run /mcp list for status.`).
- [x] `docs/ROADMAP.md` stale 정리 1차 — 상단 callout으로 v0.1 출시 후 작업은 TODO/CHANGELOG 참조 + demo seed 항목 [x]. — commit `5245892`.
- [ ] `docs/ROADMAP.md` 본문 unchecked 항목 재분류 — done/deferred/superseded 명시.
- [ ] workspace import/export 확장 — 이슈/run/comment/run_event/attachment 직렬화 + PII 마스킹 정책 + HTTP `/api/workspaces/:slug/export` 라우트.
- [ ] Auto-chain UI 후속 — chain 단위 cancel/retry 운영 액션, chain depth/cost guard 임계치 시각화, workspace 차원 chain dashboard.
- [ ] 첨부 파일 후속 — comment 첨부, image inline preview, MIME deep-sniff / 바이러스 스캔, 외부 storage(S3/MinIO), 다운로드 audit log.
- [ ] webhook 후속 — issue.cancelled 이벤트, 다회 retry + exponential backoff + dead-letter, payload masking 옵션.
- [ ] per-run worktree 후속 — `git worktree add/remove` 통합, worktree 파일 자동 시드, 디스크 사용량 관측/GC.
