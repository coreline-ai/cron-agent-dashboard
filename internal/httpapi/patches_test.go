...
// Per-run worktree opt-in can be selected during workspace creation from the UI.
func TestCreateWorkspaceRespectsPerRunWorktreeTrue(t *testing.T) {
	dir := t.TempDir()
	database, err := db.OpenAndMigrate(filepath.Join(dir, "data.db"))
	if err != nil {
		t.Fatal(err)
	...
	body := `{"name":"PerRunWT","slug":"per-run-wt","identifier_prefix":"PRW","per_run_worktree":true,"main_agent":{"name":"Lead","runtime":"codex","instructions":"lead"}}`
	res := do(t, h, http.MethodPost, "/api/workspaces", body)
	if res.Code != http.StatusCreated {
		t.Fatalf("create status=%d body=%s", res.Code, res.Body.String())
	}
	...
	if !created.Workspace.PerRunWorktree {
		t.Fatalf("per_run_worktree=true should be honored")
	}
}
